...
def format_time(time_str):
    # time: "00:00:27" to "000027"
    parts = time_str.split(":")
    formatted_time = f"{int(parts[0]):02d}{int(parts[1]):02d}{int(float(parts[2])):02d}"
...
def download_video(name, video_id, start_time, duration_time):
    ...
    output_file = os.path.join(file_path, name + "-" + video_id + "-" + start_time_formatted + ".mp4")
